...
(define (make-ordered-deck)
  ...
  (se (make-suit 'H) (make-suit 'S) (make-suit 'D) (make-suit 'C) 'JO1 'JO2))
...
(define (image-card? card)
  (and (not (joker-card? card)) (or (equal? (first card) 'J) (equal? (first card) 'Q) (equal? (first card) 'K))))

(define (joker-card? card)
  (equal? (bl card) 'JO))
