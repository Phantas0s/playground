...
func serveFiles(res http.ResponseWriter, req *http.Request) {
	mutex.RLock() //RLock are for readers - no multiple reads on the same map
	v, found := cache[req.URL.Path]
	mutex.RUnlock()

	if !found {
		mutex.Lock() //Lock reading and writing in the cache
		defer mutex.Unlock()
		...
		r := bytes.NewReader(b.Bytes()) // content need to implements io.ReadSeeker for ServeContent
